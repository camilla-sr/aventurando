...
create table if not exists TENDENCIAS(
	id_tend				int unsigned auto_increment primary key,
    tendencia			varchar(25) not null unique
);

create table if not exists ANTECEDENTES(
	id_antec			int unsigned auto_increment primary key,
...
create table if not exists BUGIGANGAS(
	id_bugig			int unsigned auto_increment primary key,
...
create table if not exists HAB_MAGIA(
	id_hab				int unsigned auto_increment primary key,
    hab_chave			varchar(30) not null
);

# TABELAS COM FK
create table if not exists ARMADURAS(
	id_armor			int unsigned auto_increment primary key,
...
create table if not exists FERRAMENTAS(
	id_ferram			int unsigned auto_increment primary key,
...
create table if not exists HABILIDADES_C(
	id_habC				int unsigned auto_increment primary key,
...
create table if not exists ARQUETIPOS(
	id_arque			int unsigned auto_increment primary key,
    i_classe			int unsigned not null,
...
create table if not exists HABILIDADES_R(
	id_habR				int unsigned auto_increment primary key,
...
create table if not exists USUARIO_MAGICO(
	id_usuario			int unsigned auto_increment primary key,
...
create table if not exists ARMAS(
	id_weapon			int unsigned auto_increment primary key,
...
create table if not exists UTILITARIOS(			# Armazena apenas itens da tebela de ferramentas
	id_utilit			int unsigned auto_increment primary key,
...
create table if not exists TRANQUEIRAS(			# Armazena apenas itens da tabela de bugigangas
	id_tranq			int unsigned auto_increment primary key,
    i_bugig				int unsigned not null,
    quantidade				int(3) default 0,
    constraint foreign key (i_bugig) references BUGIGANGAS(id_bugig)
);

create table if not exists TRUQUES(
    id_truque		 	int unsigned auto_increment primary key,
    i_usuario		 	int unsigned not null,
...
create table if not exists JOGADOR(				# TELA 1
    ...
    constraint foreign key (i_usuario) references USUARIO(id_usario),
    constraint foreign key (i_raca) references RACAS(id_raca),
    constraint foreign key (i_classe) references CLASSES(id_classe),
...
create table if not exists DETALHES(				# TELA 2
